## Universal Code Review Instructions
  ...
  - Amp check/agent mode or new thread must **never** post comments; only the main agent posts comments.
  - Use `gh` for GitHub interactions and `git` for local diff/context.
  - Do not use Amp UI input or equivalent user-interrupt flow.
  ...
  For each candidate issue, run a validator Amp check/agent mode or new thread:
  ...
  ### Fallback: No Amp check/agent mode or new thread Available

  If Amp check/agent mode or new thread are unavailable, perform the same process sequentially yourself:
  - Policy pass #1
  - Policy pass #2
  - Bug pass #1
  - Bug pass #2
