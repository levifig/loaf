...
export default {
      ...
      for (const [matcher, hookList] of Object.entries(preToolHooks)) {
        if (matchesTool(toolName, matcher)) {
          for (const hook of hookList) {
            const result = runHook('pre-tool', toolName, hook.id, hook.command, hook.script, hook.timeout, hook.failClosed);
    ...
    'tool.result': async (input) => {
      ...
      for (const [matcher, hookList] of Object.entries(postToolHooks)) {
        if (matchesTool(toolName, matcher)) {
          for (const hook of hookList) {
            const result = runHook('post-tool', toolName, hook.id, hook.command, hook.script, hook.timeout, hook.failClosed);
