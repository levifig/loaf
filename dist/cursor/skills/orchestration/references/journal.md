...
- SessionStart emits the layered continuity digest; background agent invocations exit
...
- background agent are suppressed using the normalized target agent identity. A parent agent should record the semantic conclusion when it consumes a material background agent finding; raw background agent traces do not become project journal churn.
...
| Pass a session alias to background agent | Nothing to pass — the harness id is automatic |
