## Universal Code Review Instructions
  ...
  - background agent must **never** post comments; only the main agent posts comments.
  - Use `gh` for GitHub interactions and `git` for local diff/context.
  - Do not use built-in chat clarification or equivalent user-interrupt flow.
  ...
  For each candidate issue, run a validator background agent:
  ...
  ### Fallback: No background agent Available

  If background agent are unavailable, perform the same process sequentially yourself:
  - Policy pass #1
  - Policy pass #2
  - Bug pass #1
  - Bug pass #2
