...
`--require-executable` turns the executability report into a gate (exit code 1 if not executable) — this is implement's preflight and CI's non-draft-PR check, not something shape itself passes during shaping.
