## Universal Code Review Instructions
  ...
  - subtask agent must **never** post comments; only the main agent posts comments.
  - Use `gh` for GitHub interactions and `git` for local diff/context.
  - Do not use prompt the user in chat or equivalent user-interrupt flow.
  ...
  For each candidate issue, run a validator subtask agent:
  ...
  ### Fallback: No subtask agent Available

  If subtask agent are unavailable, perform the same process sequentially yourself:
  - Policy pass #1
  - Policy pass #2
  - Bug pass #1
  - Bug pass #2
