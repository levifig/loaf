...
3. **subtask agent absorb exploration.** Use subtask agent for broad investigation and return concise findings to the main context.
...
| Multi-file exploration | Explorer or research subtask agent |
| Implementation work | Implementer or task-focused agent |
| Long audit | Background agent with report output |

Pass stable references to subtask agent: Change IDs, task IDs, branch names, and report paths. The harness ID is attached to journal entries automatically; there is no session alias to pass.
