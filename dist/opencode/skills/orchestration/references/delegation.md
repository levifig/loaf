...
| Track tasks | native task/todo surface when available |
| Manage external issues | Linear, GitHub |
| Read files for context | Read, Grep, Glob |
| Ask clarifying questions | prompt the user in chat (OpenCode: `question`) |
| Assign subtask agent work | native task/todo surface when available (subtask agent read via native task/todo surface when available) |
...
+-- Complex Decision?
    +-- Council (5-7 subtask agent, odd number)
```
...
4. **Reference durable artifacts** - Task, spec, and report IDs; the subtask agent's journal entries are harness-id tagged automatically
