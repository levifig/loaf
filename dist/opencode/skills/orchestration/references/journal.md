...
- SessionStart emits the layered continuity digest; subtask agent invocations exit
...
- subtask agent are suppressed using the normalized target agent identity. A parent agent should record the semantic conclusion when it consumes a material subtask agent finding; raw subtask agent traces do not become project journal churn.
...
| Pass a session alias to subtask agent | Nothing to pass — the harness id is automatic |
