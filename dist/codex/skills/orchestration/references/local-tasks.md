...
> **SQLite-backed state model:** In migrated projects, SQLite is the operational
> source of truth for task metadata (status, priority, dependencies, dates, and
> relationships). Task Markdown files hold authored prose and compatibility
> context. `TASKS.json` is a migration/compatibility index, not the mutation
> surface. Use `loaf task` CLI commands for all metadata operations.
...
the allocation is recorded in state; compatibility indexes may mirror it during
...
| Rule | Guideline |
|------|-----------|
| **One agent type** | Task completable by ONE separate Codex thread or explicit multi-agent tool when available (implementer, reviewer, researcher) |
