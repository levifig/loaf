...
| Agent | Focus | Use For |
|-------|-------|---------|
| `{{AGENT:backend-dev}}` | Backend | Services, APIs, business logic, backend code review |
| `{{AGENT:frontend-dev}}` | Frontend | UI components, state management, frontend code review |
| `rails-dev` | Ruby on Rails | Controllers, models, views, Hotwire/Stimulus, ActiveRecord |
| `{{AGENT:dba}}` | Database | Schema design, migrations, indexes, query optimization |
| `{{AGENT:devops}}` | Infrastructure | Docker, Kubernetes, CI/CD, monitoring |

### Quality Assurance Agents

| Agent | Focus | Use For |
|-------|-------|---------|
| `{{AGENT:qa}}` | Tests | Unit tests, integration tests, fixtures, coverage |
...
| `{{AGENT:design}}` | UI/UX | Interface design, accessibility, design systems |
...
|-- Code Implementation
|   |-- Python/FastAPI/Backend --> {{AGENT:backend-dev}}
|   |-- React/Next.js/Frontend --> {{AGENT:frontend-dev}}
|   |-- Ruby on Rails --> rails-dev
|   +-- Database Schema/Migrations --> {{AGENT:dba}}

|-- Infrastructure & Operations
|   |-- Docker/K8s/CI/CD --> {{AGENT:devops}}
|   +-- Database Performance --> {{AGENT:dba}}

|-- Quality Assurance
|   |-- Test Implementation --> {{AGENT:qa}}
|   +-- Security Audit --> security

|-- Code Review (Domain Dev)
|   |-- Backend Review --> {{AGENT:backend-dev}}
|   +-- Frontend Review --> {{AGENT:frontend-dev}}
...
```python
# Step 1: Schema first
Task(subagent_type="{{AGENT:dba}}", prompt="Create users table...")

# Wait for completion

# Step 2: Implementation uses schema
Task(subagent_type="{{AGENT:backend-dev}}", prompt="Implement user service...")

# Wait for completion

# Step 3: Tests use implementation
Task(subagent_type="{{AGENT:qa}}", prompt="Write user tests...")
```
...
Task(subagent_type="{{AGENT:backend-dev}}", prompt="Implement API...")
Task(subagent_type="{{AGENT:frontend-dev}}", prompt="Build UI...")
```
...
2. **One concern per agent** - Don't ask {{AGENT:backend-dev}} to also write tests
...
| Asking {{AGENT:backend-dev}} for React | Spawn {{AGENT:frontend-dev}} |
| Single agent for database + backend + tests | Sequential: {{AGENT:dba}}, {{AGENT:backend-dev}}, {{AGENT:qa}} |
