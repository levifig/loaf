...
4. If no `session:` field exists:
   ...
   This task hasn't been started yet. Use {{IMPLEMENT_CMD}} TASK-XXX to begin work.
   ```
...
Usage:
  {{RESUME_CMD}} TASK-001        # Resume by task ID
  {{RESUME_CMD}} <session-file>  # Resume by session file
```
...
Usage:
  {{RESUME_CMD}} TASK-001        # Resume by task ID
  {{RESUME_CMD}} <session-file>  # Resume by session file
Example: {{RESUME_CMD}} 20251215-115340-sdk-foundation
```
