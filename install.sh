...
main() {
    ...
    # Step 4: Ensure loaf binary is available on PATH
    if ! command -v loaf &>/dev/null; then
        ...
        cat > "${LOCAL_BIN}/loaf" << EOF
#!/usr/bin/env bash
# Loaf CLI wrapper - auto-generated by install.sh
SCRIPT_DIR="\$(cd "\$(dirname "\${BASH_SOURCE[0]}")" && pwd)"
REPO_DIR="${repo_dir}"
if [[ -f "\${REPO_DIR}/dist-cli/index.js" ]]; then
    node "\${REPO_DIR}/dist-cli/index.js" "\$@"
else
    echo "error: Loaf not found at \${REPO_DIR}" >&2
    exit 1
fi
EOF
        chmod +x "${LOCAL_BIN}/loaf"
        ok "Installed loaf binary to ${LOCAL_BIN}"
    ...
    # Step 5: Install to detected tools (pass through any flags)
    if [[ ${#install_args[@]} -gt 0 ]]; then
        node dist-cli/index.js install "${install_args[@]}"
    else
        # Interactive mode - no args means user gets prompted
