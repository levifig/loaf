...
| Fetch task | Get issue | `loaf task show TASK-XXX` (or read TASKS.json directly) |
...
> **Managed .md Model:** TASKS.json (`.agents/TASKS.json`) is the source of truth for
> structured metadata (status, priority, dependencies, dates). Task .md files hold body
> content (description, acceptance criteria, work log). Frontmatter in .md files is a
> read-only mirror synced from JSON. Use `loaf task` CLI commands for all metadata operations.
...
Task IDs are auto-generated by `loaf task create` using the `next_id` counter in TASKS.json.
...
```bash
loaf task update TASK-001 --session YYYYMMDD-HHMMSS-description.md
```
