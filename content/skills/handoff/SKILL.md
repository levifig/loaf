...
description: >-
  ...
  agent, branch, task, or future session. Use when the user asks "handoff
  this," "prepare the next session," "write transfer context," or when work is
  being parked for later. Not for routine session continuity (use
  orchestration) or session shutdown (use wrap). Produces a disposable handoff
