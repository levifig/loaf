...
Release archives also carry `loaf-release-manifest.json`, a deterministic manifest of the platform binary and every file in the Amp distribution. External Amp consumer repositories can commit the strict pin and lifecycle templates under [`content/templates/amp-orb`](../../content/templates/amp-orb/): setup verifies the selected archive checksum before extraction, installs it in a per-project Orb user prefix, activates only that binary, installs project-local Amp content, and runs `loaf harness readiness`. Readiness verifies the archive receipt, release manifest, binary, distribution, project declaration, canonical Git remote, ownership manifests, PATH resolution, duplicate global/project Loaf sources, and Amp project/workspace claims observed through a short-lived lifecycle identity token without retaining it. This consumer path is distinct from this repository's own `.agents/setup`, which remains a source-checkout development bootstrap and is not a template for projects that merely consume Loaf.
