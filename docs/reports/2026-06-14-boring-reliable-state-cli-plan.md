...
The next implementation commit should finish the remaining `state export` failure-mode pass by sampling missing/invalid state, unsupported kinds, unsupported formats, and `--json` misuse across all export kinds, then normalize the first inconsistent JSON or human next-action gap it finds.
