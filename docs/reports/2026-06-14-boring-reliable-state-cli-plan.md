...
The next implementation commit should continue the completion-audit pass by auditing backend/Linear export and repair follow-up paths from the primary checkout with isolated XDG homes, then tighten the first unclear policy, JSON, or human-output contract that appears.
