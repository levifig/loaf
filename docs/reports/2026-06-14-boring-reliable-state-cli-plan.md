...
The next implementation commit should continue the completion-audit pass by sampling invalid-state project-command output, especially schema drift and project identity mismatch cases, and tightening the first error path whose next action or context is less clear than the documented reliability contract.
