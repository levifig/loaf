...
The next implementation commit should continue the completion-audit pass by auditing remaining non-control-plane `--agent-help` raw JSON descriptions, prioritizing SQLite-backed task/spec/session/report surfaces whose JSON contracts are already stable.
