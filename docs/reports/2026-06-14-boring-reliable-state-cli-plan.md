...
The next implementation commit should continue the dogfood human-output matrix across `state status`, `state init`, and `state doctor` in both healthy and missing-state modes, then normalize the first inconsistent high-risk surface it finds.
