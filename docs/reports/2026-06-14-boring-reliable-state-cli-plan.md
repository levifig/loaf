...
The next implementation commit should finish the export/report pass by sampling unsupported-format and invalid/missing-state failure modes for `state export` and `report generate`, then normalize the first inconsistent JSON or human next-action gap it finds.
