...
The next implementation commit should continue the dogfood human-output matrix across remaining state surfaces, starting with `state path` and backup command syntax/output, then normalize the first inconsistent high-risk command family it finds.
