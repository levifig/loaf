...
- **U3 — Vocabulary normalization parity.** `CanonicalLifecycleStatus` at insert for tasks, reports, ideas, brainstorms; raw-plus-warning for out-of-vocabulary values. Shaping drafts are excluded — no lifecycle vocabulary exists for that kind; they get insert-only status from U1 and nothing more.
