...
- **Release gate:** A release whose ancestry contains any materialized node of `change-model-hard-cut` is refused until `spec-conversion-and-guidance-sweep` is also in that ancestry. Releases cut from ancestry predating the first merged node are unaffected. The resulting freeze window is accepted and has no hotfix override; `release-after` is immutable dependency metadata, not lifecycle state.
...
- **No-gos:** No automatic semantic disposition, no interactive CLI wizard, no new workflow entity, no mutation of other projects, no release, and no guidance sweep that makes the still-present legacy recovery path undiscoverable.
...
Capture and delivery are equally uneven. Manual entries commonly have no harness session ID despite guidance claiming automatic correlation. Missing-state automatic logging intentionally exits silently. Git and PR hooks infer completion from command text without requiring successful results or durable IDs. OpenCode and Amp generated plugins run continuity commands but discard stdout, so the model receives nothing. Loaf installs no Codex lifecycle hooks even though the installed Codex runtime now exposes stable SessionStart/UserPromptSubmit/Stop hooks. Equal event names are not equal outcomes.
...
3. **No release occurs between lineage nodes.** Once release ancestry contains the first materialized lineage node, release preflight freezes publication until that ancestry also contains the named terminal node; ancestry predating the lineage remains unaffected. The freeze is accepted and `release-after` is immutable dependency metadata, not mutable lineage status.
...
20. **Unsupported Codex hook semantics get an explicit no-op adapter.** Codex `0.144.1` rejects Loaf's legacy top-level `version` and flat hook records, and Loaf has not yet proven an equivalent translation for its `failClosed`, `blocking`, and command-condition semantics. The Codex build therefore emits the valid current-schema fallback `{"hooks":{}}`; install retires only recognized legacy flat Loaf records, preserves valid nested user matcher groups and top-level description values, and refuses malformed, unknown, or unsupported user content. U7/U8 own a real normalized Codex hook adapter after runtime fidelity is proven.
...
|--------|------------------|----------------------------------|-------------------|----------|
| Claude Code | Generated lifecycle commands and Claude-shaped payload handling exist; installed model-visible smoke still required | Unknown until real PostToolUse payload fixtures prove exit/result semantics | Native SessionStart/PostCompact delivery plus result-aware PostToolUse only if the signal is proven | Managed instruction to run `loaf journal context`; explicit CLI; no automatic completion when unproven |
...
| Codex | Installed `codex-cli 0.144.1` reports stable hooks, accepts SessionStart/UserPromptSubmit/Stop, and parses user-layer execpolicy rules whose exact `allow` prefixes run outside the workspace sandbox without prompting; its runtime rejects Loaf's legacy flat hook projection, so the current build installs a valid empty hook fallback | Unknown; SessionStart capability does not imply PostToolUse result fidelity | Install an explicit opt-in basic-command policy from the central registry, retain hardened journal guidance, and spike SessionStart stdout/model injection, normalize Codex payload, generate native start if proven; classify compaction and completion separately | Valid empty current-schema hooks plus managed path-pinned basic-policy guidance and explicit CLI; body/file-consuming and path-taking leaves remain gated; no automatic completion when unproven; fail visibly when the managed policy is absent, conflicted, or bypassed |
| OpenCode | Current plugin invokes commands but discards stdout; current compaction injection surface differs | Unknown until adapter receives a result-bearing event | Inject initial context through a real prompt/system transformation and append resumption context through the supported compaction adapter; completion only if separately proven | Managed instruction plus explicit CLI; no automatic completion when unproven |
| Amp | Current `session.start` projection runs unrelated hooks and discards stdout | Unknown until the chosen tool event exposes a durable result | Use an agent-start model-context message for the bounded digest; session-start only for non-model initialization; completion only if separately proven | Managed instruction plus explicit CLI; no automatic completion when unproven |
...
- **U8 — Target-specific delivery adapters.** Prove Claude/Cursor behavior, spike and implement Codex SessionStart or explicit fallback, correct OpenCode prompt/compaction delivery, replace Amp startup projection with agent-context delivery, and record versioned supported/bypass capability evidence.
...
- **Remaining boundary:** U7 per-target namespaced origin fixtures and normalized hook-envelope behavior remain outstanding; U8 still owns a proven model-visible Codex lifecycle adapter beyond the explicit empty fallback.
...
- **V11.** Target adapter unit tests assert model-visible returned/injected context and warning routing, not merely event registration. Installed-runtime smokes cover supported Claude Code, Cursor, Codex, OpenCode, and Amp versions/modes; each capability record independently names context-delivery and completion-capture support plus the tested version/mechanism. Any unproven/bypass mode emits the explicit fallback and is absent from the corresponding automatic-capability claim.
...
- Change lineage cannot pass structural executability with missing/cyclic/forked ancestry or release during the accepted freeze; discovery survives branch deletion; successor activation freshly verifies predecessor behavior because structural ancestry is not completion; no lineage entity/status was introduced.
...
- [KU] Does Codex SessionStart command stdout enter model-visible context on the installed stable hook runtime, and what compaction event is separately available? → U8 installed runtime spike; native adapter if proven, explicit managed-instruction start read otherwise.
- [KU] Which exact current OpenCode and Amp adapter APIs preserve model context, warnings, session identity, and compaction/resumption behavior across installed versions? → U8 official API/source check plus installed smoke; unsupported versions fall back explicitly.
...
- Installed Codex evidence: `codex-cli 0.144.1`, stable `hooks` feature, accepted local SessionStart/UserPromptSubmit/Stop configuration, official Rules/Auto-review/Hooks documentation, local `codex execpolicy check` parsing, and isolated positive/negative Auto-mode runs. The runtime rejected Loaf's legacy flat PreToolUse file, accepted the generated empty current-schema fallback, wrote and retrieved one journal entry only with the managed exact-prefix rule, and permission-blocked the same write when that rule was absent.
