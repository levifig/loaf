...
Provenance: Decisions 1–5 originate in the accepted 2026-07-10 journal-first and hard-cut conversations. Decisions 6–18 were selected after isolated adversarial scenarios, three independent read-only reviews, current source inspection, and installed Codex capability inspection; they remain reviewable through this Change and the final Claude critique.
...
| Codex | Installed `codex-cli 0.144.1` reports stable hooks and accepts SessionStart/UserPromptSubmit/Stop; Loaf currently emits only PreToolUse | Unknown; SessionStart capability does not imply PostToolUse result fidelity | Spike SessionStart stdout/model injection, normalize Codex payload, generate native start if proven; classify compaction and completion separately | Managed instruction that explicitly requires initial context read plus explicit CLI; no automatic completion when unproven |
...
Managed fences include a content fingerprint independent of package version. Build manifests record target capability/adapters and hashes for managed instructions/hooks/plugins. Install compares desired and installed fingerprints even when the version marker matches, removes stale managed projections, and reports user-owned conflicts without overwriting them.
...
- **Committed checkpoints:** U1 lineage execution guard: `b7f4d556`; U2 explicit project identity: `2d7c352e`; U3 journal/index integrity and durability: `31993521`; U4 recovery tiers and rehearsal: `803366f4`.
...
- **Final verification gates:** `LOAF_VALIDATE_TYPESCRIPT=1 npm run build` passed all targets with Go artifact synchronization; `npm run test` and typecheck passed. Executable Change check passed with only the expected unmaterialized `release-after` warning. Render-drift, ephemeral-provenance, changed-text secret, and validate-commit checks passed. An isolated public smoke proved manual origin capture, atomic defer/search, current schema, and backup plus verify recovery/provenance readiness.
- **Remaining boundary:** U7 per-target namespaced origin fixtures and normalized hook-envelope behavior remain outstanding.
...
- Installed Codex evidence: `codex-cli 0.144.1`, stable `hooks` feature, and accepted local SessionStart/UserPromptSubmit/Stop configuration; Loaf's Codex build/install output currently supplies only PreToolUse enforcement hooks.
