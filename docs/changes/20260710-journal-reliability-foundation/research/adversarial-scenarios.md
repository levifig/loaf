...
| Merge the first lineage node, then attempt an unrelated release before the terminal node | An unscoped `release-after` rule either freezes every release forever or is ignored outside the Change branch | P1 contract gap | Freeze stable and normal-semver releases only when their ancestry contains any node of the lineage but not its named terminal; ancestry predating the first node remains unaffected; allow only explicit prerelease dogfood from an already-prerelease version, with no generic hotfix override |
...
Release gating is ancestry-scoped: a stable or normal-semver release containing any lineage node but not the named terminal is refused, while ancestry predating the first node is unaffected. The accepted hard-cut lineage deliberately freezes those releases during that window; explicit prerelease dogfood may advance an already-prerelease version, with no generic hotfix override.
...
| `release-after` had no defined ancestry scope | Accepted: freeze begins when release ancestry first contains any lineage node and ends when it contains the terminal for stable/normal-semver publication; older ancestry is unaffected; only explicit prerelease dogfood from an already-prerelease version is allowed, with no generic hotfix override |
