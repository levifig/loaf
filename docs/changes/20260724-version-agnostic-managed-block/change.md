...
6. **Doctor's check becomes `fenced-content`, needs no version provenance, and distinguishes drift from tamper.** It recomputes the fingerprint of the section's actual body (never trusting the stored marker sha for the pass verdict) and compares it against the body constant compiled into the running binary: match → pass, mismatch → drift warning with `loaf install --upgrade` as the remedy. When the stored marker sha disagrees with the actual body, doctor reports tamper (the section was hand-edited) with its own message, mirroring install's refusal. Header form does not gate the verdict — legacy-stamped sections get the same content comparison, and the pending stamp-strip transition is not itself a warning (the next upgrade performs it). Decision 3 of `20260718-installed-distribution-authority` (skip fenced comparison when provenance is unresolvable) becomes moot for this check — content comparison works regardless; the no-AGENTS.md and no-section skip/warn paths stay.
...
- **V5.** `loaf doctor` on the smoke project passes `fenced-content` after install, warns drift after a simulated content change (old body seeded), warns tamper when the stored marker sha disagrees with the actual body, and `doctor --json` reports `contract_version: 2` with no output mentioning fenced version drift.
- **V6.** Old-binary fail-closed, executed: the previously released binary (installed brew `loaf`, currently v2.0.0-alpha.13) run against a transitioned smoke project errors with the malformed-fingerprint refusal and leaves the file byte-identical.
