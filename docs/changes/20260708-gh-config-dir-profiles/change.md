...
- **`GH_TOKEN` injection** (hook rewrite or otherwise) — puts token material in process environments when a non-secret selector achieves the same isolation; superseded by profiles.
...
| `git push` over HTTPS, wired session | credential helper inherits `GH_CONFIG_DIR` → correct identity *(spike-verified before this row ships)* |
