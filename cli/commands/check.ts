...
async function validateCommit(context: HookContext): Promise<CheckResult> {
  ...
  // Block AI-attribution patterns only (trailers, attribution verbs, bot footers).
  // Bare AI-tool names appear in legitimate contexts — target identifiers,
  // harness descriptions, model references in changelog entries — and must
  // not be blocked.
  const aiTools = "claude|gpt|copilot|chatgpt|gemini|anthropic|openai";
  const aiAttributionPatterns = [
    // Git trailer: "Co-authored-by: Claude <noreply@anthropic.com>"
    new RegExp(`^\\s*co-authored-by:\\s*[^\\n]*\\b(?:${aiTools})\\b`, "im"),
    // Attribution verbs near AI names: "Generated by Claude", "Created with Anthropic",
    // "Authored by GPT-4", "Written with Copilot"
    new RegExp(`\\b(?:generated|created|authored|written|produced)\\s+(?:by|with|using)\\s+[^\\n]*\\b(?:${aiTools})\\b`, "i"),
    // Bot emoji + AI name footer: "🤖 Generated with [Claude Code]"
    new RegExp(`🤖[^\\n]*\\b(?:${aiTools})\\b`, "i"),
  ];
  if (aiAttributionPatterns.some((pattern) => pattern.test(message))) {
    result.passed = false;
    result.blocked = true;
    result.errors.push(
      "Commit message contains AI attribution (trailer, attribution verb, or bot footer). Remove the attribution before committing."
    );
  }
