...
1. Switch to the base branch and pull (session already closed by `/loaf:wrap` in Step 4b):
   ```bash
...
4. Delete the merged feature branch locally and remotely:
   ...
   ```
5. **Run `/loaf:reflect`** — always run after merge, already on the base branch. Reflect looks back at the shipped work and updates strategic docs (VISION.md, STRATEGY.md, ARCHITECTURE.md) with learnings. Use `AskUserQuestion` to confirm before running.
