...
func nativeAmpHeader(version string) string {
	return `// @i-know-the-amp-plugin-api-is-wip-and-very-experimental-right-now
/**
...
func nativeAmpPluginBody() string {
    ...
    'tool.call': async (input) => {
      ...
      const toolInput = call.arguments;
      if (!toolName) return;
        ...
        if (matchesTool(toolName, matcher)) {
          for (const hook of hookList) {
            ...
            if (result.exitCode === 2) {
              return { error: result.stderr, block: true };
            }
