...
func TestClaudeInstalledSmokeEvidenceRejectsPlatformSwappedNativeBinaryPath(t *testing.T) {
	receiptPath := filepath.Join(testRepositoryRoot(t), "docs/changes/20260710-journal-reliability-foundation/research/claude-code-2.1.209-plugin-startup-smoke.json")
...
func TestCodexInstalledSmokeEvidenceRejectsPlatformSwappedNativeBinaryPath(t *testing.T) {
	receiptPath := filepath.Join(testRepositoryRoot(t), "docs/changes/20260710-journal-reliability-foundation/research/codex-0.144.4-isolated-startup-smoke.json")
	raw := readSmokeReceiptRaw(t, receiptPath)
	artifacts := raw["candidate_artifacts"].(map[string]any)
	sourceNativePath := artifacts["native_binary_path"].(string)
...
func TestInstalledSmokeEvidenceRejectsCrossTargetNativeBinaryPaths(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			target:     "claude-code",
			receipt:    "claude-code-2.1.209-plugin-startup-smoke.json",
			modeName:   "startup",
		...
		},
		{
			...
			target:     "codex",
			receipt:    "codex-0.144.4-isolated-startup-smoke.json",
			modeName:   "startup",
...
func TestExactCapabilityVersionGrammar(t *testing.T) {
	valid := []string{"2.1.209", "3.11.19", "2026.05.09-0afadcc", "0.144.4", "1.17.18", "0.0.1783873056-g278461", "1.2.3-alpha9"}
