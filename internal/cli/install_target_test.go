...
// A desired Amp build that ships only loaf.ts treats an unmanifested exact
// historical loaf-modes.ts digest as a retireable predecessor, not as a live
// artifact. One extra byte keeps the file foreign so loaf.ts can still converge.
func TestInstallTargetAdapterManifestRetiresUnmanifestedExactAmpModesPredecessor(t *testing.T) {
	...
	hookDecision := findArtifactPlanDecision(t, decisions, "plugin:.amp/plugins/loaf.ts")
	if hookDecision.Action != planActionCreate {
		t.Fatalf("loaf.ts plan action = %#v, want create", hookDecision)
	}
	if err := installTargetDistribution(options); err != nil {
	...
	if got := findArtifactPlanDecision(t, secondPlan, "plugin:.amp/plugins/loaf.ts"); got.Action != planActionPreserve {
		t.Fatalf("second loaf.ts plan action = %#v, want preserve", got)
	}
}
...
func TestInstallTargetAdapterManifestPreservesModifiedUnmanifestedAmpModesPredecessor(t *testing.T) {
	...
	writeInstallFile(t, foreign, "export const company = true;\n")
	writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{
		{"id": "plugin:.amp/plugins/loaf.ts", "kind": "plugin", "source_path": ".amp/plugins/loaf.ts", "destination": "plugins/loaf.ts", "sha256": sha256Hex(firstHook)},
	})
	options := targetInstallOptions{Target: "amp", DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: home}
	if err := installTargetDistribution(options); err != nil {
		t.Fatalf("initial Amp loaf.ts install error = %v", err)
	}

	writeInstallFile(t, hookSource, updatedHook)
	writeInstallFile(t, modesDest, modified)
	writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{
		{"id": "plugin:.amp/plugins/loaf.ts", "kind": "plugin", "source_path": ".amp/plugins/loaf.ts", "destination": "plugins/loaf.ts", "sha256": sha256Hex(updatedHook)},
	})
	decisions, err := planTargetAdapterArtifacts(options)
	if err != nil {
		t.Fatalf("planTargetAdapterArtifacts error = %v", err)
	...
	if got := findArtifactPlanDecision(t, decisions, "plugin:.amp/plugins/loaf.ts"); got.Action != planActionUpdate {
		t.Fatalf("loaf.ts plan action = %#v, want update", got)
	}
	if err := installTargetDistribution(options); err != nil {
