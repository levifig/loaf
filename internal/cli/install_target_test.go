...
func TestInstallTargetAdapterManifestPreservesForeignConvergesAndRejectsTampering(t *testing.T) {
	...
	dist := filepath.Join(root, "dist", "opencode")
	config := filepath.Join(root, "config", "opencode")
	pluginSource := filepath.Join(dist, "plugins", "hooks.js")
	pluginDest := filepath.Join(config, "plugins", "hooks.js")
	...
	writeTestTargetAdapterManifest(t, dist, "opencode", []map[string]string{{
		"id":          "runtime-plugin",
		"kind":        "plugin",
		"source_path": "plugins/hooks.js",
		"destination": "plugins/hooks.js",
...
func TestInstallTargetAdapterManifestMigratesAndRetiresAmpPlugin(t *testing.T) {
	...
	sourcePath := filepath.Join(dist, ".amp", "plugins", "loaf.js")
	destination := filepath.Join(config, "plugins", "loaf.js")
	...
	writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{{
		"id": "plugin:.amp/plugins/loaf.js", "kind": "plugin", "source_path": ".amp/plugins/loaf.js", "destination": "plugins/loaf.js", "sha256": sha256Hex(newPlugin),
	}})
...
func TestInstallTargetAdapterManifestMigratesOwnedTypeScriptAdapters(t *testing.T) {
	root := realpath(t, t.TempDir())
	home := filepath.Join(root, "home")
	dist := filepath.Join(root, "dist", "amp")
	config := filepath.Join(root, "amp")
	oldPlugin := "/**\n * Amp Plugin - Agent Skills Hooks\n * Auto-generated by loaf build system\n */\nold\n"
	newPlugin := "/**\n * Amp Plugin - Agent Skills Hooks\n * Auto-generated by loaf build system\n */\nnew\n"
	writeInstallFile(t, filepath.Join(dist, ".amp", "plugins", "loaf.js"), newPlugin)
	writeInstallFile(t, filepath.Join(config, "plugins", "loaf.ts"), oldPlugin)
	writeInstallFile(t, filepath.Join(config, "plugins", "company.ts"), "company\n")
	writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{{
		"id": "plugin:.amp/plugins/loaf.js", "kind": "plugin", "source_path": ".amp/plugins/loaf.js", "destination": "plugins/loaf.js", "sha256": sha256Hex(newPlugin),
	}})
	...
	if err := writeTargetAdapterManifest(filepath.Join(config, targetInstallManifestFile), targetAdapterManifest{
		Version: 1, Target: "amp", PackageVersion: "9.8.7-test.1", CapabilityContractVersion: 3,
		...
		t.Fatalf("write leftover TypeScript ownership: %v", err)
	}
	options := targetInstallOptions{Target: "amp", DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: home}

	decisions, err := planTargetAdapterArtifacts(options)
	if err != nil {
		t.Fatalf("owned leftover TypeScript plan error = %v", err)
	}
	legacy := findArtifactPlanDecision(t, decisions, ampHookPluginPredecessorID)
	if legacy.Action != planActionRetire {
		t.Fatalf("owned leftover TypeScript plan action = %#v, want retire", legacy)
	}
	created := findArtifactPlanDecision(t, decisions, ampHookPluginArtifactID)
	if created.Action != planActionCreate {
		t.Fatalf("JavaScript successor plan action = %#v, want create", created)
	}
	if err := installTargetDistribution(options); err != nil {
		t.Fatalf("owned leftover TypeScript adapter migration error = %v", err)
	}
	assertInstallFile(t, filepath.Join(config, "plugins", "loaf.js"), newPlugin)
	...
	if _, ok := targetAdapterArtifactsByID(installed.Artifacts)[ampHookPluginPredecessorID]; ok {
		t.Fatalf("installed artifacts = %#v, want TypeScript predecessor retired", installed.Artifacts)
	}
	if _, ok := targetAdapterArtifactsByID(installed.Artifacts)[ampHookPluginArtifactID]; !ok {
		t.Fatalf("installed artifacts = %#v, want JavaScript successor owned", installed.Artifacts)
	}

	if err := installTargetDistribution(options); err != nil {
		t.Fatalf("repeated JavaScript adapter upgrade error = %v", err)
	}
	assertInstallFile(t, filepath.Join(config, "plugins", "loaf.js"), newPlugin)
	assertInstallPathMissing(t, filepath.Join(config, "plugins", "loaf.ts"))
}

func TestInstallTargetAdapterManifestConflictsOnUnrecordedGeneratedTypeScriptAdapters(t *testing.T) {
	ampHeader := "/**\n * Amp Plugin - Agent Skills Hooks\n * Auto-generated by loaf build system\n */\n"
	...
	for _, tc := range []struct {
		...
		{name: "amp", target: "amp", sourcePath: ".amp/plugins/loaf.js", destination: "plugins/loaf.js", successor: ampHookPluginArtifactID, legacyID: ampHookPluginPredecessorID, legacyDest: "plugins/loaf.ts", header: ampHeader},
		{name: "amp-modified", target: "amp", sourcePath: ".amp/plugins/loaf.js", destination: "plugins/loaf.js", successor: ampHookPluginArtifactID, legacyID: ampHookPluginPredecessorID, legacyDest: "plugins/loaf.ts", header: ampHeader, modified: true},
		{name: "opencode", target: "opencode", sourcePath: "plugins/hooks.js", destination: "plugins/hooks.js", successor: openCodeHookPluginArtifactID, legacyID: openCodeHookPluginPredecessorID, legacyDest: "plugins/hooks.ts", header: openCodeHeader},
		{name: "opencode-modified", target: "opencode", sourcePath: "plugins/hooks.js", destination: "plugins/hooks.js", successor: openCodeHookPluginArtifactID, legacyID: openCodeHookPluginPredecessorID, legacyDest: "plugins/hooks.ts", header: openCodeHeader, modified: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			desired := tc.header + "new\n"
			leftover := tc.header + "old\n"
			if tc.modified {
				leftover += "local-edit\n"
			}
			writeInstallFile(t, filepath.Join(dist, filepath.FromSlash(tc.sourcePath)), desired)
			writeInstallFile(t, filepath.Join(config, filepath.FromSlash(tc.legacyDest)), leftover)
			writeInstallFile(t, filepath.Join(config, "plugins", "company.ts"), "company\n")
			writeTestTargetAdapterManifest(t, dist, tc.target, []map[string]string{{
				"id": tc.successor, "kind": "plugin", "source_path": tc.sourcePath, "destination": tc.destination, "sha256": sha256Hex(desired),
			}})
			options := targetInstallOptions{Target: tc.target, DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: home}

			decisions, err := planTargetAdapterArtifacts(options)
			if err != nil {
				t.Fatalf("planTargetAdapterArtifacts error = %v", err)
			}
			legacy := findArtifactPlanDecision(t, decisions, tc.legacyID)
			if legacy.Action != planActionConflict || !strings.Contains(legacy.Detail, "not proven Loaf-owned") {
				t.Fatalf("unrecorded generated TypeScript plan = %#v, want conflict", legacy)
			}
			if err := installTargetDistribution(options); err == nil || !strings.Contains(err.Error(), "not proven Loaf-owned") {
				t.Fatalf("unrecorded generated TypeScript install error = %v, want conflict", err)
			}
			assertInstallFile(t, filepath.Join(config, filepath.FromSlash(tc.legacyDest)), leftover)
			assertInstallPathMissing(t, filepath.Join(config, filepath.FromSlash(tc.destination)))
			assertInstallFile(t, filepath.Join(config, "plugins", "company.ts"), "company\n")
...
func TestInstallTargetAdapterManifestRefusesLatePredecessorAfterCleanup(t *testing.T) {
	for _, tc := range []struct {
		name        string
		target      string
		sourcePath  string
		...
		{name: "amp-file", target: "amp", sourcePath: ".amp/plugins/loaf.js", destination: "plugins/loaf.js", successor: ampHookPluginArtifactID, legacyDest: "plugins/loaf.ts", inject: "file"},
		{name: "amp-dangling-symlink", target: "amp", sourcePath: ".amp/plugins/loaf.js", destination: "plugins/loaf.js", successor: ampHookPluginArtifactID, legacyDest: "plugins/loaf.ts", inject: "symlink"},
		{name: "opencode-file", target: "opencode", sourcePath: "plugins/hooks.js", destination: "plugins/hooks.js", successor: openCodeHookPluginArtifactID, legacyDest: "plugins/hooks.ts", inject: "file"},
		{name: "opencode-dangling-symlink", target: "opencode", sourcePath: "plugins/hooks.js", destination: "plugins/hooks.js", successor: openCodeHookPluginArtifactID, legacyDest: "plugins/hooks.ts", inject: "symlink"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			root := realpath(t, t.TempDir())
			home := filepath.Join(root, "home")
			dist := filepath.Join(root, "dist", tc.target)
			config := filepath.Join(root, "config", tc.target)
			desired := "export const successor = true;\n"
			writeInstallFile(t, filepath.Join(dist, filepath.FromSlash(tc.sourcePath)), desired)
			writeTestTargetAdapterManifest(t, dist, tc.target, []map[string]string{{
				"id": tc.successor, "kind": "plugin", "source_path": tc.sourcePath, "destination": tc.destination, "sha256": sha256Hex(desired),
			}})
			...
			options := targetInstallOptions{
				Target: tc.target, DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: home,
				TargetAdapterOps: &targetAdapterInstallOperations{beforeArtifact: func(id string) error {
					...
					case "symlink":
						if err := os.Symlink(filepath.Join(root, "missing-legacy.ts"), legacyPath); err != nil {
							t.Fatalf("Symlink late predecessor: %v", err)
						}
					}
					return nil
				}},
			...
			if err == nil {
				t.Fatal("late predecessor install error = nil, want conflict")
			}
			if tc.inject == "file" {
				if !strings.Contains(err.Error(), "not proven Loaf-owned") && !strings.Contains(err.Error(), "changed during install") {
					t.Fatalf("late predecessor file error = %v, want conflict or race", err)
				}
				assertInstallFile(t, legacyPath, "late leftover\n")
			} else {
				if !strings.Contains(err.Error(), "dangling symlink") && !strings.Contains(err.Error(), "symlink") && !strings.Contains(err.Error(), "changed during install") {
					t.Fatalf("late predecessor symlink error = %v, want symlink conflict or race", err)
				}
				info, statErr := os.Lstat(legacyPath)
				if statErr != nil || info.Mode()&os.ModeSymlink == 0 {
					t.Fatalf("late predecessor Lstat = %v info=%v, want preserved dangling symlink", statErr, info)
				}
			}
			assertInstallPathMissing(t, filepath.Join(config, filepath.FromSlash(tc.destination)))
			assertInstallPathMissing(t, filepath.Join(config, targetInstallManifestFile))
...
func TestInstallTargetAdapterManifestPreservesRecreatedPredecessorOnRollback(t *testing.T) {
	for _, tc := range []struct {
		name   string
		inject string
	}{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			writeInstallFile(t, filepath.Join(config, "plugins", "loaf.ts"), oldPlugin)
			writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{{
				"id": ampHookPluginArtifactID, "kind": "plugin", "source_path": ".amp/plugins/loaf.js", "destination": "plugins/loaf.js", "sha256": sha256Hex(newPlugin),
			}})
			legacyMode := uint32(0o644)
			if err := writeTargetAdapterManifest(filepath.Join(config, targetInstallManifestFile), targetAdapterManifest{
				Version: 1, Target: "amp", PackageVersion: "9.8.7-test.1", CapabilityContractVersion: 3,
			...
			options := targetInstallOptions{
				Target: "amp", DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: home,
				TargetAdapterOps: &targetAdapterInstallOperations{afterArtifact: func(id string) error {
					if id != ampHookPluginArtifactID {
						return nil
					}
			...
			err := installTargetDistribution(options)
			if err == nil {
				t.Fatal("recreated predecessor install error = nil, want conflict")
			}
			if !strings.Contains(err.Error(), "not proven Loaf-owned") && !strings.Contains(err.Error(), "dangling symlink") && !strings.Contains(err.Error(), "symlink") {
				t.Fatalf("recreated predecessor error = %v, want predecessor conflict", err)
			}
			if tc.inject == "file" {
				assertInstallFile(t, legacyPath, "late leftover\n")
			} else {
			...
			assertInstallPathMissing(t, filepath.Join(config, "plugins", "loaf.js"))
...
func TestInstallTargetAdapterManifestPreservesPredecessorInjectedAfterRemoval(t *testing.T) {
	for _, tc := range []struct {
		name    string
		present bool
	}{
		{name: "initially-present", present: true},
		{name: "initially-absent", present: false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			root := realpath(t, t.TempDir())
			home := filepath.Join(root, "home")
			...
			options := targetInstallOptions{
				Target: "amp", DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: home,
				TargetAdapterOps: &targetAdapterInstallOperations{afterArtifact: func(id string) error {
					if id != ampHookPluginPredecessorID {
						return nil
					}
					writeInstallFile(t, legacyPath, "late leftover\n")
					return nil
			...
			err := installTargetDistribution(options)
			if err == nil {
				t.Fatal("post-removal leftover install error = nil, want conflict")
			}
			if !strings.Contains(err.Error(), "not proven Loaf-owned") && !strings.Contains(err.Error(), "changed during install") {
				t.Fatalf("post-removal leftover error = %v, want predecessor conflict", err)
			}
			assertInstallFile(t, legacyPath, "late leftover\n")
			assertInstallPathMissing(t, filepath.Join(config, "plugins", "loaf.js"))
			installed, readErr := readTargetAdapterManifest(filepath.Join(config, targetInstallManifestFile))
...
func TestInstallTargetAdapterManifestDoesNotInferOpenCodePredecessorForCodex(t *testing.T) {
	root := realpath(t, t.TempDir())
	home := filepath.Join(root, "home")
	dist := filepath.Join(root, "dist", "codex")
	config := filepath.Join(root, "config", "codex")
	desired := "export const hooks = 1;\n"
	leftover := "export const leftover = true;\n"
	writeInstallFile(t, filepath.Join(dist, "plugins", "hooks.js"), desired)
	writeInstallFile(t, filepath.Join(config, "plugins", "hooks.ts"), leftover)
	writeTestTargetAdapterManifest(t, dist, "codex", []map[string]string{{
		"id": openCodeHookPluginArtifactID, "kind": "plugin", "source_path": "plugins/hooks.js", "destination": "plugins/hooks.js", "sha256": sha256Hex(desired),
	}})
	options := targetInstallOptions{Target: "codex", DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: home}
	if _, ok := targetAdapterJavaScriptSuccessor(options.Target, targetAdapterArtifact{ID: openCodeHookPluginArtifactID}); ok {
		t.Fatal("Codex plugin:plugins/hooks.js inferred an OpenCode TypeScript predecessor")
	}
	if err := syncTargetAdapterManifest(options); err != nil {
		t.Fatalf("Codex JavaScript plugin sync error = %v", err)
	}
	assertInstallFile(t, filepath.Join(config, "plugins", "hooks.js"), desired)
	assertInstallFile(t, filepath.Join(config, "plugins", "hooks.ts"), leftover)
	installed, err := readTargetAdapterManifest(filepath.Join(config, targetInstallManifestFile))
	if err != nil {
		t.Fatalf("read installed Codex ownership: %v", err)
	}
	if _, ok := targetAdapterArtifactsByID(installed.Artifacts)[openCodeHookPluginPredecessorID]; ok {
		t.Fatalf("installed artifacts = %#v, want no OpenCode TypeScript predecessor row", installed.Artifacts)
	}
	for _, id := range installed.RetiredArtifactIDs {
		if id == openCodeHookPluginPredecessorID {
			t.Fatalf("retired artifact ids = %#v, want no OpenCode TypeScript tombstone", installed.RetiredArtifactIDs)
		}
	}
}

func TestInstallTargetAmpRefusesManifestlessJavaScriptCopy(t *testing.T) {
	root := realpath(t, t.TempDir())
	home := filepath.Join(root, "home")
	dist := filepath.Join(root, "dist", "amp")
	config := filepath.Join(root, "amp")
	desired := "export const loaf = 'js';\n"
	legacy := "export const loaf = 'ts';\n"
	writeInstallFile(t, filepath.Join(dist, ".amp", "plugins", "loaf.js"), desired)
	writeInstallFile(t, filepath.Join(config, "plugins", "loaf.ts"), legacy)
	options := targetInstallOptions{Target: "amp", DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: home}
	if err := installTargetDistribution(options); err == nil || !strings.Contains(err.Error(), "target adapter manifest") {
		t.Fatalf("manifestless Amp install error = %v, want ownership refusal", err)
	}
	assertInstallFile(t, filepath.Join(config, "plugins", "loaf.ts"), legacy)
	assertInstallPathMissing(t, filepath.Join(config, "plugins", "loaf.js"))
	assertInstallPathMissing(t, filepath.Join(config, targetInstallManifestFile))
}

func TestInstallTargetAdapterManifestConflictsOnUnownedTypeScriptAdapters(t *testing.T) {
	root := realpath(t, t.TempDir())
	home := filepath.Join(root, "home")
	dist := filepath.Join(root, "dist", "amp")
	config := filepath.Join(root, "amp")
	desired := "/**\n * Amp Plugin - Agent Skills Hooks\n * Auto-generated by loaf build system\n */\nnew\n"
	foreign := "export const userPlugin = true;\n"
	writeInstallFile(t, filepath.Join(dist, ".amp", "plugins", "loaf.js"), desired)
	writeInstallFile(t, filepath.Join(config, "plugins", "loaf.ts"), foreign)
	writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{{
		"id": "plugin:.amp/plugins/loaf.js", "kind": "plugin", "source_path": ".amp/plugins/loaf.js", "destination": "plugins/loaf.js", "sha256": sha256Hex(desired),
	}})
	options := targetInstallOptions{Target: "amp", DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: home}
	...
	legacy := findArtifactPlanDecision(t, decisions, ampHookPluginPredecessorID)
	if legacy.Action != planActionConflict || !strings.Contains(legacy.Detail, "not proven Loaf-owned") {
		t.Fatalf("unowned TypeScript plan = %#v, want conflict", legacy)
	}
	if err := installTargetDistribution(options); err == nil || !strings.Contains(err.Error(), "not proven Loaf-owned") {
		t.Fatalf("unowned TypeScript install error = %v, want conflict", err)
	}
	assertInstallFile(t, filepath.Join(config, "plugins", "loaf.ts"), foreign)
	assertInstallPathMissing(t, filepath.Join(config, "plugins", "loaf.js"))
	assertInstallPathMissing(t, filepath.Join(config, targetInstallManifestFile))
}

func TestInstallTargetAdapterManifestConflictsOnSymlinkedTypeScriptAdapters(t *testing.T) {
	root := realpath(t, t.TempDir())
	home := filepath.Join(root, "home")
	dist := filepath.Join(root, "dist", "amp")
	config := filepath.Join(root, "amp")
	...
	writeInstallFile(t, filepath.Join(dist, ".amp", "plugins", "loaf.js"), desired)
	writeInstallFile(t, outside, desired)
	mkdirAll(t, filepath.Join(config, "plugins"))
	if err := os.Symlink(outside, filepath.Join(config, "plugins", "loaf.ts")); err != nil {
		t.Fatalf("Symlink leftover TypeScript adapter: %v", err)
	}
	writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{{
		"id": "plugin:.amp/plugins/loaf.js", "kind": "plugin", "source_path": ".amp/plugins/loaf.js", "destination": "plugins/loaf.js", "sha256": sha256Hex(desired),
	}})
	...
	legacy := findArtifactPlanDecision(t, decisions, ampHookPluginPredecessorID)
	if legacy.Action != planActionConflict || !strings.Contains(legacy.Detail, "symlink") {
		t.Fatalf("symlinked TypeScript plan = %#v, want symlink conflict", legacy)
	}
	if err := installTargetDistribution(options); err == nil || !strings.Contains(err.Error(), "symlink") {
		t.Fatalf("symlinked TypeScript install error = %v, want conflict", err)
	}
	info, err := os.Lstat(filepath.Join(config, "plugins", "loaf.ts"))
	if err != nil || info.Mode()&os.ModeSymlink == 0 {
		t.Fatalf("leftover TypeScript adapter Lstat = %v info=%v, want preserved symlink", err, info)
	}
	assertInstallPathMissing(t, filepath.Join(config, "plugins", "loaf.js"))
}

// A desired Amp build that ships only loaf.js treats an unmanifested exact
// historical loaf-modes.ts digest as a retireable predecessor, not as a live
// artifact. One extra byte keeps the file foreign so loaf.js can still converge.
func TestInstallTargetAdapterManifestRetiresUnmanifestedExactAmpModesPredecessor(t *testing.T) {
	root := realpath(t, t.TempDir())
	home := filepath.Join(root, "home")
	dist := filepath.Join(root, "dist", "amp")
	config := filepath.Join(root, "amp")
	hookSource := filepath.Join(dist, ".amp", "plugins", "loaf.js")
	hookDest := filepath.Join(config, "plugins", "loaf.js")
	...
	writeInstallFile(t, foreign, "export const company = true;\n")
	writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{
		{"id": ampHookPluginArtifactID, "kind": "plugin", "source_path": ".amp/plugins/loaf.js", "destination": "plugins/loaf.js", "sha256": sha256Hex(hookBody)},
	})
	options := targetInstallOptions{Target: "amp", DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: home}
	...
	if modesDecision.Action != planActionRetire {
		t.Fatalf("unmanifested predecessor plan action = %#v, want retire", modesDecision)
	}
	hookDecision := findArtifactPlanDecision(t, decisions, ampHookPluginArtifactID)
	if hookDecision.Action != planActionCreate {
		t.Fatalf("loaf.js plan action = %#v, want create", hookDecision)
	}
	if err := installTargetDistribution(options); err != nil {
		t.Fatalf("Amp predecessor retirement error = %v", err)
	}
	assertInstallFile(t, hookDest, hookBody)
	assertInstallPathMissing(t, modesDest)
	assertInstallFile(t, foreign, "export const company = true;\n")
	assertInstalledAmpManifestOmitsModes(t, config)

	afterFirst := string(readFileBytes(t, hookDest))
	if err := installTargetDistribution(options); err != nil {
		t.Fatalf("second Amp predecessor retirement error = %v", err)
	}
	assertInstallFile(t, hookDest, afterFirst)
	assertInstallPathMissing(t, modesDest)
	assertInstallFile(t, foreign, "export const company = true;\n")
	assertInstalledAmpManifestOmitsModes(t, config)
	secondPlan, err := planTargetAdapterArtifacts(options)
	if err != nil {
		t.Fatalf("second planTargetAdapterArtifacts error = %v", err)
	}
	for _, decision := range secondPlan {
		if decision.ID == ampModesPluginArtifactID {
			t.Fatalf("second plan still classified modes = %#v, want no-op without a modes row", decision)
		}
	}
	if got := findArtifactPlanDecision(t, secondPlan, ampHookPluginArtifactID); got.Action != planActionPreserve {
		t.Fatalf("second loaf.js plan action = %#v, want preserve", got)
	}
}

func TestInstallTargetAdapterManifestPreservesModifiedUnmanifestedAmpModesPredecessor(t *testing.T) {
	root := realpath(t, t.TempDir())
	home := filepath.Join(root, "home")
	dist := filepath.Join(root, "dist", "amp")
	config := filepath.Join(root, "amp")
	...
	modesDest := filepath.Join(config, "plugins", "loaf-modes.ts")
	foreign := filepath.Join(config, "plugins", "company.ts")
	firstHook := "export const loafHooks = 1;\n"
	updatedHook := "export const loafHooks = 2;\n"
	...
	writeInstallFile(t, hookSource, firstHook)
	writeInstallFile(t, foreign, "export const company = true;\n")
	writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{
		{"id": ampHookPluginArtifactID, "kind": "plugin", "source_path": ".amp/plugins/loaf.js", "destination": "plugins/loaf.js", "sha256": sha256Hex(firstHook)},
	})
	options := targetInstallOptions{Target: "amp", DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: home}
	if err := installTargetDistribution(options); err != nil {
		t.Fatalf("initial Amp loaf.js install error = %v", err)
	}

	writeInstallFile(t, hookSource, updatedHook)
	writeInstallFile(t, modesDest, modified)
	writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{
		{"id": ampHookPluginArtifactID, "kind": "plugin", "source_path": ".amp/plugins/loaf.js", "destination": "plugins/loaf.js", "sha256": sha256Hex(updatedHook)},
	})
	decisions, err := planTargetAdapterArtifacts(options)
	if err != nil {
		t.Fatalf("planTargetAdapterArtifacts error = %v", err)
	...
	for _, decision := range decisions {
		if decision.ID == ampModesPluginArtifactID {
			t.Fatalf("modified unmanifested predecessor plan = %#v, want no retire", decision)
		}
	}
	if got := findArtifactPlanDecision(t, decisions, ampHookPluginArtifactID); got.Action != planActionUpdate {
		t.Fatalf("loaf.js plan action = %#v, want update", got)
	}
	if err := installTargetDistribution(options); err != nil {
		t.Fatalf("Amp modified predecessor install error = %v", err)
	}
	assertInstallFile(t, hookDest, updatedHook)
	assertInstallFile(t, modesDest, modified)
	assertInstallFile(t, foreign, "export const company = true;\n")
	assertInstalledAmpManifestOmitsModes(t, config)
}

func TestInstallTargetAdapterManifestPreservesUnmanifestedAmpModesSymlink(t *testing.T) {
	root := realpath(t, t.TempDir())
	home := filepath.Join(root, "home")
	dist := filepath.Join(root, "dist", "amp")
	config := filepath.Join(root, "amp")
	...
	hookDest := filepath.Join(config, "plugins", "loaf.js")
	modesDest := filepath.Join(config, "plugins", "loaf-modes.ts")
	foreign := filepath.Join(root, "foreign-modes.ts")
	hookBody := "export const loafHooks = 1;\n"
	writeInstallFile(t, hookSource, hookBody)
	writeInstallFile(t, foreign, testAmpModesPredecessor(t))
	if err := os.MkdirAll(filepath.Dir(modesDest), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink(foreign, modesDest); err != nil {
		t.Fatal(err)
	}
	writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{{
		"id": ampHookPluginArtifactID, "kind": "plugin", "source_path": ".amp/plugins/loaf.js", "destination": "plugins/loaf.js", "sha256": sha256Hex(hookBody),
	...
	for _, decision := range decisions {
		if decision.ID == ampModesPluginArtifactID {
			t.Fatalf("foreign symlink classified as modes = %#v", decision)
		}
	}
	if err := installTargetDistribution(options); err != nil {
		t.Fatalf("Amp upgrade with foreign modes symlink error = %v", err)
	}
	assertInstallFile(t, hookDest, hookBody)
	if target, err := os.Readlink(modesDest); err != nil || target != foreign {
		t.Fatalf("foreign modes symlink = %q, %v; want %q", target, err, foreign)
	}
	assertInstallFile(t, foreign, testAmpModesPredecessor(t))
	assertInstalledAmpManifestOmitsModes(t, config)
}

func TestInstallTargetAdapterManifestRetiresAndRefusesPreviouslyManagedAmpModesStub(t *testing.T) {
	t.Run("retire matching stub", func(t *testing.T) {
		root := realpath(t, t.TempDir())
		home := filepath.Join(root, "home")
		dist := filepath.Join(root, "dist", "amp")
		config := filepath.Join(root, "amp")
		...
		stub := "modes-stub\n"
		writeInstallFile(t, hookSource, hookBody)
		writeInstallFile(t, modesSource, stub)
		writeInstallFile(t, foreign, "export const company = true;\n")
		writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{
			{"id": ampModesPluginArtifactID, "kind": "plugin", "source_path": ampModesPluginSourcePath, "destination": ampModesPluginDestination, "sha256": sha256Hex(stub)},
		...
		options := targetInstallOptions{Target: "amp", DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: home}
		if err := installTargetDistribution(options); err != nil {
			t.Fatalf("initial managed Amp modes stub install error = %v", err)
		}
		...
		if got := findArtifactPlanDecision(t, decisions, ampModesPluginArtifactID); got.Action != planActionRetire {
			t.Fatalf("previously managed stub plan action = %#v, want retire", got)
		}
		if err := installTargetDistribution(options); err != nil {
			t.Fatalf("previously managed Amp modes stub retirement error = %v", err)
		}
		assertInstallFile(t, hookDest, hookBody)
		assertInstallPathMissing(t, modesDest)
		assertInstallFile(t, foreign, "export const company = true;\n")
	...
	t.Run("conflict on modified stub", func(t *testing.T) {
		root := realpath(t, t.TempDir())
		home := filepath.Join(root, "home")
		dist := filepath.Join(root, "dist", "amp")
		config := filepath.Join(root, "amp")
		...
		writeInstallFile(t, modesDest, "modes-stub edited\n")
		writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{
			{"id": "plugin:.amp/plugins/loaf.ts", "kind": "plugin", "source_path": ".amp/plugins/loaf.ts", "destination": "plugins/loaf.ts", "sha256": sha256Hex(hookBody)},
		})
		decisions, err := planTargetAdapterArtifacts(options)
		...
		if got := findArtifactPlanDecision(t, decisions, ampModesPluginArtifactID); got.Action != planActionConflict {
			t.Fatalf("modified previously managed stub plan action = %#v, want conflict", got)
		}
		if err := installTargetDistribution(options); err == nil || !strings.Contains(err.Error(), "modified") {
			t.Fatalf("modified previously managed stub error = %v, want modified-content conflict", err)
		}
		assertInstallFile(t, hookDest, hookBody)
		assertInstallFile(t, modesDest, "modes-stub edited\n")
		assertInstallFile(t, foreign, "export const company = true;\n")
	})
}

func TestInstallTargetAdapterManifestRetiresAmpModesPredecessorInConfiguredPluginsDir(t *testing.T) {
	root := realpath(t, t.TempDir())
	home := filepath.Join(root, "home")
	dist := filepath.Join(root, "dist", "amp")
	config := filepath.Join(root, "amp")
	pluginsDir := filepath.Join(root, "amp-plugins")
	hookSource := filepath.Join(dist, ".amp", "plugins", "loaf.ts")
	hookDest := filepath.Join(pluginsDir, "loaf.ts")
	modesDest := filepath.Join(pluginsDir, "loaf-modes.ts")
	decoy := filepath.Join(config, "plugins", "loaf-modes.ts")
	foreign := filepath.Join(pluginsDir, "company.ts")
	hookBody := "export const loafHooks = 1;\n"
	predecessor := testAmpModesPredecessor(t)
	writeInstallFile(t, hookSource, hookBody)
	writeInstallFile(t, modesDest, predecessor)
	writeInstallFile(t, decoy, "config decoy\n")
	writeInstallFile(t, foreign, "export const company = true;\n")
	writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{
		{"id": "plugin:.amp/plugins/loaf.ts", "kind": "plugin", "source_path": ".amp/plugins/loaf.ts", "destination": "plugins/loaf.ts", "sha256": sha256Hex(hookBody)},
	})
	options := targetInstallOptions{Target: "amp", DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: home, AmpPluginsDir: pluginsDir}
	...
	if got := findArtifactPlanDecision(t, decisions, ampModesPluginArtifactID); got.Action != planActionRetire {
		t.Fatalf("configured-dir predecessor plan action = %#v, want retire", got)
	}
	if err := installTargetDistribution(options); err != nil {
		t.Fatalf("configured Amp plugins dir predecessor retirement error = %v", err)
	}
	assertInstallFile(t, hookDest, hookBody)
	assertInstallPathMissing(t, modesDest)
	assertInstallFile(t, decoy, "config decoy\n")
	assertInstallFile(t, foreign, "export const company = true;\n")
	assertInstalledAmpManifestOmitsModes(t, config)
}

func TestInstallTargetAdapterManifestRollsBackAmpModesPredecessorRetirement(t *testing.T) {
	root := realpath(t, t.TempDir())
	home := filepath.Join(root, "home")
	dist := filepath.Join(root, "dist", "amp")
	config := filepath.Join(root, "amp")
	...
	retiredPredecessor := 0
	options := targetInstallOptions{
		Target: "amp", DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: home, AmpPluginsDir: pluginsDir,
		TargetAdapterOps: &targetAdapterInstallOperations{beforeArtifact: func(id string) error {
			if id == ampModesPluginArtifactID {
				return nil
			}
	...
	if err := installTargetDistribution(options); !os.IsPermission(err) {
		t.Fatalf("Amp predecessor retirement rollback error = %v, want permission injection", err)
	}
	if retiredPredecessor != 1 {
		t.Fatalf("loaf.ts observations after predecessor retirement = %d, want 1", retiredPredecessor)
	}
	assertInstallPathMissing(t, hookDest)
	assertInstallFile(t, modesDest, predecessor)
	assertInstallFile(t, foreign, "export const company = true;\n")
	assertInstallPathMissing(t, filepath.Join(config, targetInstallManifestFile))
}
...
func TestInstallTargetAdapterManifestManagesIndependentAmpPlugins(t *testing.T) {
	...
	config := filepath.Join(root, "amp")
	hookSource := filepath.Join(dist, ".amp", "plugins", "loaf.js")
	modesSource := filepath.Join(dist, ".amp", "plugins", "loaf-modes.js")
	hookDest := filepath.Join(config, "plugins", "loaf.js")
	modesDest := filepath.Join(config, "plugins", "loaf-modes.js")
	foreign := filepath.Join(config, "plugins", "company.ts")
	hookBody := "export const loafHooks = 1;\n"
	...
	writeInstallFile(t, foreign, "export const company = true;\n")
	writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{
		{"id": "plugin:.amp/plugins/loaf.js", "kind": "plugin", "source_path": ".amp/plugins/loaf.js", "destination": "plugins/loaf.js", "sha256": sha256Hex(hookBody)},
		{"id": "plugin:.amp/plugins/loaf-modes.js", "kind": "plugin", "source_path": ".amp/plugins/loaf-modes.js", "destination": "plugins/loaf-modes.js", "sha256": sha256Hex(modesBody)},
	})
	options := targetInstallOptions{Target: "amp", DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: home}
...
func TestInstallTargetAdapterManifestRollsBackPartialAmpPublication(t *testing.T) {
	...
	writeInstallFile(t, filepath.Join(config, "plugins", "company.ts"), "company\n")
	writeTestTargetAdapterManifest(t, dist, "amp", []map[string]string{
		{"id": "plugin:.amp/plugins/loaf.js", "kind": "plugin", "source_path": ".amp/plugins/loaf.js", "destination": "plugins/loaf.js", "sha256": sha256Hex("hooks\n")},
		{"id": "plugin:.amp/plugins/loaf-modes.js", "kind": "plugin", "source_path": ".amp/plugins/loaf-modes.js", "destination": "plugins/loaf-modes.js", "sha256": sha256Hex("modes\n")},
	})
	modesDest := filepath.Join(config, "plugins", "loaf-modes.js")
	publishedModes := 0
	options := targetInstallOptions{
		...
		TargetAdapterOps: &targetAdapterInstallOperations{beforeArtifact: func(id string) error {
			if id == "plugin:.amp/plugins/loaf.js" {
	...
	if publishedModes != 1 {
		t.Fatalf("published loaf-modes.js observations = %d, want 1 before loaf.js failed", publishedModes)
	}
	assertInstallPathMissing(t, filepath.Join(config, "plugins", "loaf.js"))
	assertInstallPathMissing(t, filepath.Join(config, "plugins", "loaf-modes.js"))
	assertInstallPathMissing(t, filepath.Join(config, targetInstallManifestFile))
	assertInstallFile(t, filepath.Join(config, "plugins", "company.ts"), "company\n")
}
...
func TestInstallTargetAdapterManifestRejectsUnrecordedPluginCollisions(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "opencode", target: "opencode", sourcePath: "plugins/hooks.js", destination: "plugins/hooks.js"},
		{name: "amp", target: "amp", sourcePath: ".amp/plugins/loaf.js", destination: "plugins/loaf.js"},
		{name: "amp-modes", target: "amp", sourcePath: ".amp/plugins/loaf-modes.js", destination: "plugins/loaf-modes.js"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			root := realpath(t, t.TempDir())
			dist := filepath.Join(root, "dist", tc.target)
...
func TestInstallTargetAdapterManifestRejectsSymlinkedDestinationParents(t *testing.T) {
	...
	writeTestTargetAdapterManifest(t, dist, "opencode", []map[string]string{{
		"id": "plugin:plugins/hooks.js", "kind": "plugin", "source_path": "plugins/hooks.js", "destination": "plugins/hooks.js", "sha256": sha256Hex("desired\n"),
	}})
	...
	assertInstallPathMissing(t, filepath.Join(outside, "hooks.js"))
}
...
func TestInstallTargetAdapterManifestBindsConcreteArtifactModes(t *testing.T) {
	t.Run("post-build source chmod drift", func(t *testing.T) {
		...
		dist := filepath.Join(root, "dist", "opencode")
		config := filepath.Join(root, "config", "opencode")
		source := filepath.Join(dist, "plugins", "hooks.js")
		writeInstallFile(t, source, "plugin\n")
		if err := os.Chmod(source, 0o755); err != nil {
			t.Fatal(err)
		}
		writeTestTargetAdapterManifest(t, dist, "opencode", []map[string]string{{
			"id": "plugin:hooks", "kind": "plugin", "source_path": "plugins/hooks.js", "destination": "plugins/hooks.js", "sha256": sha256Hex("plugin\n"),
		}})
		...
		assertInstallPathMissing(t, filepath.Join(config, "plugins", "hooks.js"))
	})
	...
	t.Run("installed chmod drift", func(t *testing.T) {
		...
		config := filepath.Join(root, "config", "opencode")
		source := filepath.Join(dist, "plugins", "hooks.js")
		destination := filepath.Join(config, "plugins", "hooks.js")
		writeInstallFile(t, source, "plugin\n")
		if err := os.Chmod(source, 0o755); err != nil {
			t.Fatal(err)
		}
...
func TestInstallTargetAdapterManifestDetectsRaceAndRollsBackPublication(t *testing.T) {
	t.Run("post-preflight race", func(t *testing.T) {
		...
		writeTestTargetAdapterManifest(t, dist, "opencode", []map[string]string{{"id": "plugin:hooks", "kind": "plugin", "source_path": "plugins/hooks.js", "destination": "plugins/hooks.js", "sha256": sha256Hex("old\n")}})
		options := targetInstallOptions{Target: "opencode", DistDir: dist, ConfigDir: config, Version: "9.8.7-test.1", HomeDir: filepath.Join(root, "home")}
		if err := installTargetDistribution(options); err != nil {
			t.Fatal(err)
		}
		writeInstallFile(t, source, "new\n")
		writeTestTargetAdapterManifest(t, dist, "opencode", []map[string]string{{"id": "plugin:hooks", "kind": "plugin", "source_path": "plugins/hooks.js", "destination": "plugins/hooks.js", "sha256": sha256Hex("new\n")}})
...
func TestInstallTargetAmpUsesSharedAndCustomHomes(t *testing.T) {
	...
	writeInstallFile(t, filepath.Join(ampDist, ".amp", "plugins", "loaf.js"), "export default function () {}\n")
	writeInstallFile(t, filepath.Join(ampDist, ".amp", "plugins", "loaf-modes.js"), "export const modes = true;\n")
	writeTestTargetAdapterManifest(t, ampDist, "amp", []map[string]string{
		{"id": "plugin:.amp/plugins/loaf.js", "kind": "plugin", "source_path": ".amp/plugins/loaf.js", "destination": "plugins/loaf.js", "sha256": sha256Hex("export default function () {}\n")},
		{"id": "plugin:.amp/plugins/loaf-modes.js", "kind": "plugin", "source_path": ".amp/plugins/loaf-modes.js", "destination": "plugins/loaf-modes.js", "sha256": sha256Hex("export const modes = true;\n")},
	})
	...
	assertInstallFile(t, filepath.Join(ampPlugins, "loaf.js"), "export default function () {}\n")
	assertInstallFile(t, filepath.Join(ampPlugins, "loaf-modes.js"), "export const modes = true;\n")
...
func TestInstallTargetAmpDefaultsPluginToConfigPluginsDir(t *testing.T) {
	...
	assertInstallFile(t, filepath.Join(ampConfig, "plugins", "loaf.js"), "export default function () {}\n")
	assertInstallFile(t, filepath.Join(ampConfig, "plugins", "loaf-modes.js"), "export const modes = true;\n")
	assertInstallPathMissing(t, filepath.Join(legacyPlugins, "loaf.js"))
	assertInstallPathMissing(t, filepath.Join(legacyPlugins, "loaf-modes.js"))
	assertInstallFile(t, filepath.Join(ampConfig, loafInstallMarkerFile), "9.8.7-test.1\n")
...
func assertInstalledAmpManifestOmitsModes(t *testing.T, config string) {
	t.Helper()
	installed, err := readTargetAdapterManifest(filepath.Join(config, targetInstallManifestFile))
	if err != nil {
		t.Fatalf("read installed Amp ownership error = %v", err)
	}
	if _, ok := targetAdapterArtifactsByID(installed.Artifacts)[ampModesPluginArtifactID]; ok {
		t.Fatalf("installed Amp artifacts = %#v, want no loaf-modes.ts row", installed.Artifacts)
	}
}
