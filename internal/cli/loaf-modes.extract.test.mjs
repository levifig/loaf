...
const source = join(dirname(fileURLToPath(import.meta.url)), '../../content/amp/plugins/loaf-modes.js');

test('loaf-modes compatibility plugin is inert and points routing at loaf.js', async () => {
  const body = await readFile(source, 'utf8');
  assert.match(body, /Managed Amp routing now lives in loaf\.js/);
...
test('inert loaf-modes plugin default export is a no-op', async () => {
  const { default: initialize, description } = await import('../../content/amp/plugins/loaf-modes.js');
  assert.match(description, /loaf\.js/);
  const amp = {
